...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .company-name { font-size: 24px; font-weight: bold; color: {{ $brand['primary_color'] ?? '#333' }}; margin-bottom: 5px; }
        .invoice-title { font-size: 32px; font-weight: bold; color: {{ $brand['primary_color'] ?? '#0f172a' }}; }
        .invoice-number { font-size: 14px; color: #666; }
        .billing-info { margin-bottom: 30px; }
        .billing-info h3 { font-size: 12px; color: #999; text-transform: uppercase; margin-bottom: 10px; }
        .dates table { width: 300px; }
        .dates td { padding: 5px 0; }
        .dates td:last-child { text-align: right; }
        .items-table { width: 100%; border-collapse: collapse; margin-bottom: 30px; }
        .items-table th { background: #f5f5f5; padding: 12px; text-align: left; border-bottom: 2px solid #ddd; font-weight: 600; }
        ...
        .totals .total-row { border-top: 2px solid #333; font-weight: bold; font-size: 16px; }
        .notes { margin-top: 60px; padding-top: 20px; border-top: 1px solid #eee; }
        .notes h4 { font-size: 12px; color: #999; text-transform: uppercase; margin-bottom: 10px; }
        .footer { margin-top: 60px; text-align: center; color: #999; font-size: 10px; }
        .status-badge { display: inline-block; padding: 5px 15px; border-radius: 3px; font-weight: bold; text-transform: uppercase; font-size: 11px; }
        .status-paid { background: #28a745; color: white; }
        .status-unpaid { background: #f59e0b; color: #111827; }
    ...
    <div class="totals">
        <table>
            ...
            @if($invoice->discount > 0)
                <tr>
                    <td>Discount{{ $invoice->discount_type === 'percentage' ? ' (' . number_format($invoice->discount, 2) . '%)' : '' }}</td>
