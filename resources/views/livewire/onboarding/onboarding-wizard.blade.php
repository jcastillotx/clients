                            ...
                            @foreach((array)($questionnaire->questions ?? []) as $q)
                                ...
                                @if($key)
                                    <div>
                                        ...
                                        @if($type === 'textarea')
                                            <textarea rows="3" wire:model.defer="answers.{{ $key }}" class="w-full rounded-xl border border-slate-300 px-3 py-2.5 text-sm text-slate-900 placeholder:text-slate-400 focus:border-slate-900 focus:ring-1 focus:ring-slate-900 focus:outline-none transition-colors resize-y"></textarea>
                                        @elseif($type === 'select')
                                            <select wire:model.defer="answers.{{ $key }}" class="w-full rounded-xl border border-slate-300 px-3 py-2.5 text-sm text-slate-900 bg-white focus:border-slate-900 focus:ring-1 focus:ring-slate-900 focus:outline-none transition-colors">
                                            ...
                                            <div class="flex flex-wrap gap-2">
                                                @foreach($opts as $o)
                                                    <label class="inline-flex items-center gap-2 px-3 py-2 rounded-lg border border-slate-200 bg-slate-50 text-sm text-slate-700 cursor-pointer hover:bg-slate-100 transition-colors">
                                                        <input type="checkbox" wire:model.defer="answers.{{ $key }}" value="{{ $o }}" class="h-4 w-4 rounded border-slate-300 text-slate-900 focus:ring-slate-900 focus:ring-offset-0">
                                                        {{ $o }}
                                                    </label>
                                                @endforeach
                                            </div>
                                        @else
                                            <input type="text" wire:model.defer="answers.{{ $key }}" class="w-full rounded-xl border border-slate-300 px-3 py-2.5 text-sm text-slate-900 placeholder:text-slate-400 focus:border-slate-900 focus:ring-1 focus:ring-slate-900 focus:outline-none transition-colors">
                                        @endif
                                    </div>
                                @endif
                            @endforeach
