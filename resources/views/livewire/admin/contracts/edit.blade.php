    ...
    <div class="flex flex-wrap items-center justify-between gap-4 mb-6">
        ...
        <div class="flex items-center gap-3">
            @if($contract->file_path)
                <a href="{{ route('contracts.download', $contract) }}" class="rounded-lg border border-slate-300 bg-white px-4 py-2 text-sm font-semibold text-slate-900 hover:bg-slate-50 transition-colors inline-flex items-center gap-2">
            ...
            @endif
            <a href="{{ route('admin.contracts.index') }}" class="rounded-lg border border-slate-300 bg-white px-4 py-2 text-sm font-semibold text-slate-900 hover:bg-slate-50 transition-colors">
                Back to List
            </a>
        </div>
    </div>
        ...
        <div class="xl:col-span-2 space-y-6">
            ...
            <div class="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
                ...
                <div class="p-6 space-y-4">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        <div>
                            <label class="block text-xs font-semibold text-slate-600 mb-1.5">Contract Value <span class="text-rose-500">*</span></label>
                            <div class="relative">
            ...
            <div class="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
                <div class="px-6 py-4 border-b border-slate-200 bg-slate-50 flex items-center justify-between">
                    ...
                    @if($contract->file_path)
                        <a href="{{ route('contracts.preview', $contract) }}" target="_blank" class="text-sm font-semibold text-blue-600 hover:underline">
                            Preview PDF
                        </a>
                    @endif
                </div>
                <div class="p-6">
                    ...
                    @if($editable)
                        <div>
                            <label class="block text-xs font-semibold text-slate-600 mb-1.5">Replace Document (PDF, DOC, DOCX)</label>
                        ...
                        <div class="mt-4 pt-4 border-t border-slate-200">
                            <button wire:click="toggleAiPanel" class="inline-flex items-center gap-2 rounded-lg border border-blue-300 bg-blue-50 px-4 py-2 text-sm font-semibold text-blue-700 hover:bg-blue-100 transition-colors">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 20 20" fill="currentColor">
            ...
            <div class="rounded-2xl border-2 border-blue-200 bg-white shadow-sm overflow-hidden">
                ...
                <div class="p-6 space-y-4">
                    <div>
                        <label class="block text-xs font-semibold text-slate-600 mb-1.5">Contract Template <span class="text-rose-500">*</span></label>
                    ...
                    <div class="flex gap-3">
                        <button wire:click="generateWithAi" wire:loading.attr="disabled" wire:loading.class="opacity-50"
                            ...
                            <svg wire:loading wire:target="generateWithAi" class="animate-spin h-4 w-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                    ...
                    @if($aiHtml)
                        <div class="mt-4">
                            <label class="block text-xs font-semibold text-slate-600 mb-1.5">Generated Content Preview</label>
                            <div class="border border-slate-200 rounded-xl p-4 bg-slate-50 max-h-64 overflow-y-auto prose prose-sm max-w-none">
                                {!! $aiHtml !!}
                            </div>
                            <div class="mt-3 flex gap-3">
                                <button wire:click="applyAiPdf" class="inline-flex items-center gap-2 rounded-lg bg-emerald-600 px-4 py-2 text-sm font-semibold text-white hover:bg-emerald-700 transition-colors">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 20 20" fill="currentColor">
                                        <path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd" />
                                    </svg>
                                    Apply as PDF
                                </button>
                                <button wire:click="clearAiDraft" class="text-sm text-slate-600 hover:underline">Clear</button>
                            </div>
                        </div>
                    @endif
                </div>
                ...
                <div class="p-6">
                    <dl class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        ...
                        <div>
                            ...
                            <dd class="mt-1 text-sm text-emerald-900">{{ $contract->signed_at->format('M d, Y \a\t g:i A') }}</dd>
                        </div>
                        <div>
                            <dt class="text-xs font-semibold text-emerald-700 uppercase tracking-wide">Signature IP</dt>
                            <dd class="mt-1 text-sm text-emerald-900 font-mono">{{ $contract->signature_ip ?? 'N/A' }}</dd>
                        </div>
                    </dl>
                </div>
            </div>
        ...
        <div class="xl:col-span-1 space-y-6">
            ...
            <div class="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden sticky top-6">
                ...
                <div class="p-6 space-y-3">
                    ...
                    @if(!in_array($contract->status, ['terminated']))
                        <button wire:click="terminateContract" wire:confirm="Are you sure you want to terminate this contract?"
                    ...
                    <button wire:click="deleteContract" wire:confirm="Are you sure you want to delete this contract? This cannot be undone."
            ...
            <div class="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
                ...
                <div class="p-6">
                    ...
                    @endphp
                    <div class="inline-flex items-center rounded-full px-3 py-1.5 text-sm font-semibold border {{ $statusBadge }}">
                        ...
                        <div class="mt-4 pt-4 border-t border-slate-200">
                            @if($contract->daysUntilExpiration > 0)
                                <p class="text-sm text-slate-600">
                                    <span class="font-semibold">{{ $contract->daysUntilExpiration }}</span> days until expiration
                                </p>
                            @elseif($contract->daysUntilExpiration === 0)
                                <p class="text-sm text-amber-600 font-semibold">Expires today</p>
                            @else
                                <p class="text-sm text-rose-600 font-semibold">Expired {{ abs($contract->daysUntilExpiration) }} days ago</p>
                            @endif
                        </div>
                    @endif
                </div>
