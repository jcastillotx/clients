...
export function StaffGuideCategoryManager() {
  ...
  const loadCategories = useCallback(async () => {
    setLoading(true);
    try {
      const data = await fetchApi<GuideCategory[]>("/api/staff-guides/categories", undefined, {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await fetchApi(
        `/api/staff-guides/categories/${id}`,
        { method: "DELETE" },
    ...
    <div className="grid gap-6 lg:grid-cols-2">
      <Card>
        <CardHeader>
          ...
          <CardDescription>Organize internal staff guides into categories.</CardDescription>
        </CardHeader>
        <CardContent>
