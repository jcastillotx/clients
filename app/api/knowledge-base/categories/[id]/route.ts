...
export async function PATCH(request: NextRequest, context: RouteContext) {
  try {
    ...
    const validated = kbCategorySchema.partial().parse(body);
    ...
    if (validated.parentId !== undefined) patch.parent_id = validated.parentId;
...
export async function DELETE(request: NextRequest, context: RouteContext) {
  try {
    ...
    const supabase = await createClient();
    const { error } = await supabase.from("knowledge_base_categories").delete().eq("id", id);
